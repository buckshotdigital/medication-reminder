...
serve(async (req) => {
  ...
  const basePlanPriceId = Deno.env.get('STRIPE_BASE_PLAN_PRICE_ID');
  const dashboardUrl = Deno.env.get('DASHBOARD_URL') || 'http://localhost:3000';

  if (!stripeSecretKey || !basePlanPriceId) {
    return new Response(JSON.stringify({ error: 'Stripe subscription not configured' }), {
      headers: { ...cors, 'Content-Type': 'application/json' },
      status: 500,
    });
  }
  ...
  try {
    ...
    if (authError || !user) {
      console.error('[stripe-subscribe] Auth error:', authError?.message, 'token prefix:', token.substring(0, 20));
    ...
    if (!caregiver) {
      console.log('[stripe-subscribe] Auto-creating caregiver for:', user.id, user.email);
      ...
      if (insertErr || !newCg) {
        console.error('[stripe-subscribe] Caregiver insert error:', insertErr);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${dashboardUrl}/dashboard/credits?subscription=success`,
      cancel_url: `${dashboardUrl}/dashboard/credits?subscription=canceled`,
    });
